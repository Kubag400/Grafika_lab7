	...
	JMenuBar getMenuBar() {
		...
		ActionListener textureListener = e -> {
			...
			switch (itemNum) {
				...
				case 3:
					try {
						currentTexture = textureFromResource("graph/earth.jpg");
				...
				case 4:
					try {
						currentTexture = textureFromResource("graph/brick.jpg");
					} catch (IOException ex) {
						ex.printStackTrace();
					}
					break;
				case 5:
					try {
						currentTexture = textureFromResource("graph/clouds.jpg");
					} catch (IOException ex) {
						ex.printStackTrace();
					}
					break;
